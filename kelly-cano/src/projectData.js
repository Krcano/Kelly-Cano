export const projects = [
  {
    id: 1,
    title: "Weather Dashboard",
    description:
      "This app is a weather dashboard to show you the current and future forecast in the city of your choosing",
    image: "./weather-screenshot.png",
    github: "https://github.com/Krcano/Weather-Dashboard",
    deployedLink: "https://krcano.github.io/Weather-Dashboard/",
  },
  {
    id: 2,
    title: "What's for Dinner?",
    description:
      "What's for Dinner is an app that helps people make the best decision that busy people have to deal with every day, what should I make for dinner?",
    image: "./sh.png",
    github: "https://github.com/jdesgarennes/whatsForDinner",
    deployedLink: "https://jdesgarennes.github.io/whatsForDinner/",
  },
  {
      id:3,
    title: "Atlas Job Tracker",
    description:
      "There are so many websites out there to apply for jobs, it can be very hard to keep track of where you applied and  off which sites. Our site makes your job hunting  that much easier.",
    image: "./Atlas.png",
    github: "https://github.com/Krcano/Job-Tracker",
    deployedLink: "https://atlas-job-tracker.herokuapp.com/",
  },
  {
      id:4,
    title: "Tech Blog",
    description:
      "An app that allows you to create a Tech Blog so you, your friends, and colleagues can talk about the latest technologies.",
    image: "./homepage.png",
    github: "https://github.com/Krcano/Tech-Blog",
    deployedLink: "https://pacific-cliffs-94965.herokuapp.com/login",
  },
  {
      id:5,
    title: "Tech Blog",
    description:
      "An app that allows you to create a Tech Blog so you, your friends, and colleagues can talk about the latest technologies.",
    image: "./homepage.png",
    github: "https://github.com/Krcano/Tech-Blog",
    deployedLink: "https://pacific-cliffs-94965.herokuapp.com/login",
  },
];
